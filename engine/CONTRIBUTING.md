...
CI owns broad regression across required portable parallel shards, the portable serial lane's separate-runner shards, the Herdr lane, lint, invariants, the coverage guard, and stock macOS Bash compatibility in `.github/workflows/engine.yml`, at the root of the repository that holds the engine.
