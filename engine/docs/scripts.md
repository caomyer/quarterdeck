...
| `fm-pr-check.sh`         | Record validated `pr=` and `pr_head=` values, atomically arm a static merge poll, then run `fm-pr-body-fix.sh` on a GitHub PR |
| `fm-pr-body-fix.sh`      | Insert the blank lines a GitHub PR body needs around `<details>` blocks, proving nothing else changed, so a screenshot after one renders as an image |
