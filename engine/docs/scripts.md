...
| `fm-primary-scope-lib.sh` | Shared marker, plain-checkout, or installed-copy primary-home predicate for tracked hooks |
...
| `fm-project-intake.sh`   | Report what registering a captain's local checkout means: a fleet name that cannot share its worktree pool, and the uncommitted or unpushed work the fleet copy will not have |
...
| `fm-tasks-axi-lib.sh`    | Shared backlog-backend selector, done-archive path resolver, and `tasks-axi` compatibility probe |
| `fm-backlog-parse-lib.sh` | Single owner of how a backlog row reads, shared by the fleet snapshot and `fm-history.sh` |
...
| `fm-crew-dispatch.sh`    | Turn crew dispatch routing on or off, edit its rules, and set or clear the typed dispatch resolution key, for callers without an editor such as Quarterdeck |
| `fm-crew-dispatch-lib.sh` | Single owner of `config/crew-dispatch.json` validation, shared by bootstrap and `fm-crew-dispatch.sh` |
