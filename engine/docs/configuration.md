...
The app reads captain calls from the snapshot's `calls[]`, where every page presented on the task a call names as its origin already argues that call, and records the captain's answers through `bin/fm-captain-hold.sh answers --source quarterdeck`; [`captain-hold-lifecycle.md`](captain-hold-lifecycle.md#structured-read-surfaces) owns that surface.
...
`bin/fm-crew-dispatch.sh` turns the file on from that example or from empty rules, replaces its rules only with content that passes the same validation bootstrap applies, and turns it off by moving it aside rather than deleting it; its header owns the verbs.
When the file exists, bootstrap validates it with `jq` through `bin/fm-crew-dispatch-lib.sh`, the one validator.
...
`bin/fm-crew-dispatch.sh set-key` writes the `.env` line from stdin and `clear-key` removes it, under the same never-printed rule.
...
A home whose `config/presentation` says `quarterdeck` presents pages in Quarterdeck instead, so it is checked for `jq` (which `bin/fm-artifact.sh` needs) and never for `lavish-axi`; an unknown presentation value reports `PRESENTATION_INVALID` and the home is treated as `lavish`.
...
FM_SNAPSHOT_UNDATED_HOLD_AGE_DAYS=14  # floored elapsed-day threshold at which an undated captain hold (no hold-until; age from its UTC hold-set timestamp, falling back to since, counted in the captain's local days, for legacy unstamped holds) is projected as a Charted Next gate instead of a live Captain's Call; 0 applies once the computed age is non-negative
