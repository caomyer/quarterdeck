...
A home is in scope when it has `AGENTS.md`, a `bin/` directory, an existing state directory, and one of: a valid `.fm-secondmate-home` marker, a plain checkout where git-dir equals git-common-dir, or no `.git` at all.
A root with no `.git` is firstmate installed as a copy (inside an app, whose home `bin/fm-home-init.sh` mirrors it), which is primary wherever it sits.
A root carrying `.git` that git cannot read is out of scope, so a broken or unreadable task worktree never counts as primary.
