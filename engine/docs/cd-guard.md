...
## Scope: plain firstmate checkouts and installed copies

The guard fires only in a plain firstmate checkout where git-dir equals git-common-dir, or in firstmate installed as a copy with no `.git` (inside an app, whose home `bin/fm-home-init.sh` mirrors it).
...
An installed copy has no task worktree to tell apart, so a root with no `.git` skips the git test; a root whose `.git` git cannot read (git missing, a broken worktree link) stays inert.
