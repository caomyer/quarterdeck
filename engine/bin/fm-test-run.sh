...
family_for_basename() {
  case "$1" in
    ...
    fm-captain-calls.test.sh|fm-project-intake.test.sh|\
    ...
    fm-watch-served-harness.test.sh|\
    ...
    fm-claude-stop-autoarm.test.sh|fm-crew-dispatch.test.sh|fm-cursor-harness.test.sh|\
...
tests/fm-crew-dispatch.test.sh 3200
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-gate-refuse*|bin/fm-lock*|bin/fm-home-init.sh)
      printf '%s\n' session-bootstrap
      ;;
    bin/fm-bootstrap.sh)
      printf '%s\n' session-bootstrap
    ...
    bin/fm-dispatch-resolve.sh)
      printf '%s\n' "__script__:fm-dispatch-resolve.test.sh"
      ;;
    bin/fm-crew-dispatch.sh)
      printf '%s\n' "__script__:fm-crew-dispatch.test.sh"
      ;;
    bin/fm-crew-dispatch-lib.sh)
      # The one crew-dispatch validator, sourced by bin/fm-bootstrap.sh and
      # bin/fm-crew-dispatch.sh.
      printf '%s\n' session-bootstrap
      printf '%s\n' "__script__:fm-crew-dispatch.test.sh"
      ;;
    ...
    bin/fm-captain-hold.sh|bin/fm-decision-options.sh)
      # The call owner, and the retired options store that is now its shim;
      # its call listing feeds the review screen through the fleet snapshot.
      printf '%s\n' pure-contract-unit
      printf '%s\n' snapshot-bearings
      ;;
