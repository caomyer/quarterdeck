...
config/sources.json  optional external task sources (GitHub Issues today) the captain connects in Quarterdeck; LOCAL, gitignored, never holds a token, NOT inherited by secondmate homes; written only by bin/fm-sources.sh, whose header owns it and the data/sources/ and state/sources/ it keeps
...
data/                personal fleet records; LOCAL, gitignored as a whole
  ...
  <id>/notes/ <id>/files/  the notes and files a task carries beside its row (evidence, scope changes), appended to each worker's launch brief; written only by bin/fm-task-note.sh; survive teardown
...
3. **Wake queue** - when locked, drains and presents the durable wake queue without running the inactive-outcome scan inline, and prints the raw records prominently as this turn's first work queue; a clearly labeled status-event annotation may follow a valid `signal` record and includes every status line still unread at the presentation cursor, but never replaces the raw record or current-state reconciliation, and a lapsed watcher chain still surfaces here via the same guard alarm.
   ...
   A bounded `UNHANDLED REPLIES` section names every open call carrying a reply the captain made on it that nothing has recorded or re-asked after a few minutes.
...
- Task-scoped notes belong with the backlog item, and investigation findings belong in the scout report.
  When a captain message that becomes a task carries files, or evidence arrives for a task later, attach it with `bin/fm-task-note.sh add`, so whoever works the task receives it.
...
When a scout's deliverable is a visual artifact the captain will iterate on, prefer keeping that scout alive to iterate on it itself - hosting its own Lavish loop, or in a quarterdeck home re-presenting revisions while firstmate relays the captain's feedback to it verbatim - rather than tearing it down, so the scout keeps its investigation context and the captain iterates in one continuous session.
When implementation is separately authorized, promote the existing scout through `bin/fm-promote.sh` rather than creating a duplicate task, passing `--title` when the ship's objective differs from the scout's so the backlog row names what will land.
...
Treat any `UNHANDLED REPLIES` section as your own overdue work: load `captain-hold-lifecycle` and record or re-ask each call it names.
...
3. For `check:`, act on the named poll result, including merges, contribution signals, task-source signals, Relay events, process-to-event source results, and captain inbox notes; a handled inbox note is also acknowledged with `bin/fm-inbox.sh drain --ack <id>`, or it stays counted as still waiting for firstmate.
...
Relay a machine-level change a worker reports - a global install, a switched tool or developer directory, a new simulator or container - in that next reply, because it outlives the task.
...
A call whose `reply` is set carries words the captain said on it that nothing has recorded: it is never awaiting the captain, and you never infer an answer from the reply alone; `captain-hold-lifecycle` owns recording or re-asking it.
...
- `captain-hold-lifecycle` - load before treating an investigation or visual review as complete, before ending a visual review that exposed a captain decision, when recording or routing the captain's answer, when a message says the captain replied to a call, and on any `RECORD DIVERGENCE` or `UNHANDLED REPLIES` line from the wake drain.
...
- `task-sources` - load on a captain message whose first line is `Take on <source> <key> (<project>)`, on any `check: sources ...` wake, when asked to link a task to an issue in a connected task source, and before dropping or tearing down a task whose backlog row carries a `source-link:` line.
