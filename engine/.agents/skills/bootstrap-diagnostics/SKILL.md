...
- `PRESENTATION_INVALID: ...` - tell the captain the presentation setting is not one firstmate knows, and that pages go through Lavish until `config/presentation` (or `FM_PRESENTATION`) says `lavish` or `quarterdeck`; do not change the setting yourself, and continue nonvisual work.
