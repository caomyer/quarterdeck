...
1. **Gather live fleet state with one deterministic command.**
   ...
   The canonical snapshot assigns every captain hold exactly one bucket from structured fields only: `blocked` when any blocker is unresolved, else `dated` while `hold_until` is after the captain's local day, else `aged` when an undated hold has reached the configured age threshold, else `live`.
