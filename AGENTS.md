...
- How every screen reads captain calls (`src/calls.ts`), how a project's logbook reads closed work (`src/logbook.ts`), how a task's body keeps the lines its filer wrote (`src/taskbody.ts`) and where the chat places a page among messages that may carry no time (`src/chatorder.ts`, whose invariant is that a page never sits below a newer message): `pnpm test`, which needs no server.
  ...
  In chat, a call is a card where it was raised, placed by `raised_at` through `pagePlaces`, and an answer is a card where it was given, read back from the app's own answer lines (`answerOfMessage`, pinned against `review.rs` by `src/fixtures/call-messages.json`); both answer through Bearings' own form and read `callStanding`, never the first mate's prose.
  Check every state, in both themes: `FIRSTMATE_URL=http://127.0.0.1:<port> pnpm calls`; its mock (`?chat-calls`) moves `raised_at` and `updated_at` only as `fm-captain-hold.sh` does, and a check must never make it move them otherwise.
...
- Check the artifact review flow, in both themes: start Vite on your own port, then
  ...
  It opens the mock with `?artifacts`, whose pages live in `src/fixtures/review-pages` and are served only by the dev server, and checks the list, the review screen, revisions, the narrow width, accepted layout findings, the frame's sandbox, the ways in from chat and the task drawer, reviewing itself: commenting on part of a page, the draft surviving a reload, taking a comment back, and sending the review with a verdict, iterating: what a later revision answers, settling a comment, and what the list says is new, deciding: a call answered inside the page that argues it, by option, in words or not now until a day, as Bearings answers it, sent with the comments in one review, and a diagram the page owns: opening its scene, proposing changes, and how they reach the author, a sent review as a card in chat that follows it until it is settled, and the captain's own t2 on the usage-panel mock (`?usage-t2`): a place the words cannot pin down goes with a redraw of the page around it, and the author is told which row and what to open.
  ...
  A page follows the app's theme through `prefers-color-scheme` alone: `src/theme.ts` sets the window's appearance from the app's `dark` class, and WebKit answers that query from it in every frame, live.
  A `color-scheme` on the frame does not reach the page in WebKit, so never carry the theme in as a class or attribute.
...
- Start work, in a queued task's drawer, hands the task to the first mate in one message (`src-tauri/src/start.rs`, which composes it and records each ask in `data/.starts/asks.jsonl`); the first mate picks the mode, briefs and spawns, and the app never runs `fm-brief.sh` or `fm-spawn.sh`, never classifies a mode, and never writes a brief.
  Where the drawer stands is read only from the row, the ask, the outbox and the snapshot's worker (`src/start.ts`, checked by `pnpm test`), and Working only from `endpoint.status` alive, which `fm-fleet-snapshot.sh` reads through the process-level probe for every local task: never from a spawn's success, `endpoint.exists` or the pane's busy signature, which reads a shell left at a prompt as busy.
  Check every state, in both themes: start Vite on your own port, then `FIRSTMATE_URL=http://127.0.0.1:<port> pnpm starts`.
  Live, which spends model tokens and runs a real scout: `cd src-tauri && FM_E2E_HOME=<scratch home> cargo test start_e2e_live_scratch_home -- --ignored --nocapture` needs a project `hello` cloned at `projects/hello`, files a queued scout, starts it the way the button does, and checks the drawer's own derivation moves through Asked and Working in order.
  Launched is caught only if a reading lands in it: `fm-spawn.sh` commits the row to In flight last, so on a healthy spawn the agent is already alive, and `pnpm starts` covers Launched.
- Work from other task systems (GitHub Issues today) is read and written only through `engine/bin/fm-sources.sh` (`src-tauri/src/sources.rs`), the one reader of a provider and the one writer of a link, a `source-link:` line in the task's body that `fm-backlog-parse-lib.sh` reads into `source_links`; the app never writes `config/sources.json`, a task body or `data/sources/`, and `src/sources.ts` reads everything from the snapshot's `sources` and the rows.
  Taking an issue on is a message on the start-work path (`take_on` in `src-tauri/src/start.rs`), which the first mate files as a queued task; it never starts one.
  The engine proves the core against a provider that is deliberately not GitHub: `cd engine && bash tests/fm-sources.test.sh`.
  Check every state, in both themes: start Vite on your own port, then `FIRSTMATE_URL=http://127.0.0.1:<port> pnpm sources`.
