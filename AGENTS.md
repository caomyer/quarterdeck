...
- Check the artifact review flow, in both themes: start Vite on your own port, then
  ...
  When a check's screenshots go to a PR as evidence, name each image a reviewer should see by its own file path in the no-mistakes test result's `artifacts[]`, not the snapshot's review artifacts: a folder there is never attached.
