...
check((await banner.innerText()).includes("needs 4 more things"), "the checklist counts what is missing");
...
check(tools.join(",") === "jq,tasks-axi,herdr", `every missing tool is named (${tools.join(", ")})`);
const commands = await page.locator(".needs-how").allInnerTexts();
check(commands.includes("brew install jq"), "an install command is shown beside its tool");
...
check((await page.locator(".needs-says").innerText()).startsWith("BACKEND_INVALID:"), "a line the app has no shape for is shown in the first mate's words");
...
const bare = await page.locator(".needs-says").evaluate((node) => Math.round(node.getBoundingClientRect().left));
